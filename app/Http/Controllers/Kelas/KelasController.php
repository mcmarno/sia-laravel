...
class KelasController extends Controller
{
  ...
  {
    $dataKelas = Mahasiswa::select(DB::raw("sempSemId,mhsNim as Nim, mhsNama, mhsAngkatan, mhsKelompok, mhsGolongan, prodiNama"))
  ...
  {
    $dataKelas = Kelas::select(DB::raw("sempSemId, dsnNidn, dsnNip, dsnNama, mkkurKode, mkkurNama, mkkurJumlahSks, mkkurSemester,klsId, klsNama, klsGolongan"))   
        ->join('semester_prodi', 'sempId', '=', 'kelas.klsSempId')
        ->join('matakuliah_kurikulum','mkkurId','=','klsMkKurId')
        ->leftjoin('dosen', 'dosen.dsnNidn', '=', 'kelas.klsDsnNidn')             
        ->where('sempSemId','=',$request->session()->get('semId')) 
  ...
  public function kelasRegister(Request $request)
  {
    ...
    $kurikulum = Mahasiswa::select(DB::raw('mhsAngkatan, mhsKurId, mhsGolongan, krsPaketSem, sempSemId, sempId'))
      ->join('krs','krsNim','=','mhsNim')
      ->join('semester_prodi','sempId','=','krsSempId')
      ->where('mhsStatusAktif','=','1')
    ...
    if ($kurikulum->count()) {
      ...
      foreach($kurikulum as $itemKurikulum){    
        ...
        $cariKelompok = Mahasiswa::select(DB::raw('mhsKelompok, mhsGolongan'))            
        ...
        foreach($cariKelompok as $itemCariKelompok){
          ...
          $namaGolongan = $itemCariKelompok->mhsGolongan;
          ...
          foreach ($makulAktif as $itemMakulAktif) {
            $cekMakulKelas = Kelas::Select(Db::raw('klsId'))
              ...
              ->where('klsGolongan', '=', $namaGolongan)
              ->get();
            //dd($cekMakulKelas);
            if($cekMakulKelas->isEmpty()){
              ...
              $kelas->klsGolongan = $namaGolongan;
    ...
    session()->flash('success',isset($messages) ? $messages : '');
    session()->flash('error',isset($messagesError) ? $messagesError : '');
    $dataKelas = Kelas::select(DB::raw("sempSemId, dsnNidn, dsnNip, dsnNama, mkkurKode, mkkurNama, mkkurJumlahSks, klsId, klsNama,klsGolongan, mkkurSemester"))   
        ->join('semester_prodi', 'sempId', '=', 'kelas.klsSempId')        
        ->join('matakuliah_kurikulum','mkkurId','=','klsMkKurId') 
        ->leftjoin('dosen', 'dosen.dsnNidn', '=', 'kelas.klsDsnNidn')             
